...
<Warning>
`DocumentRelevanceEvaluator` in Python and `createDocumentRelevanceEvaluator` in TypeScript are deprecated. Use `RetrievalRelevanceEvaluator` or `createRetrievalRelevanceEvaluator` instead. Pass one document as `context` to preserve per-document evaluation.

| Document Relevance | Retrieval Relevance |
| --- | --- |
| `document_text` in Python or `documentText` in TypeScript | `context` |
| `unrelated` label | `irrelevant` label |

The deprecated Python class will be removed in `arize-phoenix-evals` 4.0.0. The deprecated TypeScript factory and its types will be removed in `@arizeai/phoenix-evals` 3.0.0.
</Warning>
